...
import {
    ...
    PowerupSystem,
    EnemySpawnSystem,
    ExplosionSystem, KeyboardSystem, RenderSystem, DirectorSystem,
} from "./systems";
    ...
    function gameLoop(timestamp: number) {
      ...
      if (deltaTime >= renderSystem.current.frameTime) {
        ...
        const tank = world.current.getComponent(playerId, Tank);
          // Shoot on space
          if (keys[" "] && timestamp - tank.lastShot > tank.shootCooldown) {
        ...
        if (playerMovement) {
          if (keys["w"]) playerMovement.direction = "up";
          else if (keys["s"]) playerMovement.direction = "down";
          else if (keys["a"]) playerMovement.direction = "left";
          else if (keys["d"]) playerMovement.direction = "right";
        }
